...
def get_statuses_header():
    return ','.join(['status_id', 'status_message', 'status_type', 'status_link', 'status_published',
...
def adjust_date_time(statuses_path, comments_path, shares_path, reactions_path):
    ...
    with open(statuses_path, 'w', encoding='utf-8') as file:
        ...
        for status in statuses:
            status[4] = modify_date_to_recent(status[4])
            status_to_datetime[status[0].strip()] = status[4]
